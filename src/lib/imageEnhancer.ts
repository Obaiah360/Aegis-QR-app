...
function boxBlur(src, w, h, r) {
  ...
  for (let y = 0; y < h; y++) {
    for (let c = 0; c < 3; c++) {
      ...
      for (let x = 0; x < w; x++) {
        tmp[(y * w + x) * 4 + c] = sum / d;
        sum += src[(y * w + Math.max(0, Math.min(w - 1, x + r + 1))) * 4 + c] - src[(y * w + Math.max(0, Math.min(w - 1, x - r))) * 4 + c];
      }
    }
  ...
  for (let x = 0; x < w; x++) {
    for (let c = 0; c < 3; c++) {
      ...
      for (let y = 0; y < h; y++) {
        out[(y * w + x) * 4 + c] = sum / d;
        sum += tmp[(Math.max(0, Math.min(h - 1, y + r + 1)) * w + x) * 4 + c] - tmp[(Math.max(0, Math.min(h - 1, y - r)) * w + x) * 4 + c];
      }
    }
...
function adaptiveContrast(src, w, h) {
  const TILES = 4;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const mapped = luts[ty0][tx0][lum]*(1-wx)*(1-wy) + luts[ty0][tx1][lum]*wx*(1-wy) + luts[ty1][tx0][lum]*(1-wx)*wy + luts[ty1][tx1][lum]*wx*wy;
...
function autoWhiteBalance(src) {
  ...
  for (let i = 0; i < src.length; i += 4) {
    out[i] = clamp(Math.round(src[i]*rG)); out[i+1] = clamp(Math.round(src[i+1]*gG));
    out[i+2] = clamp(Math.round(src[i+2]*bG)); out[i+3] = src[i+3];
  }
  return out;
}

function finalAdjust(src, contrast, brightness) {
  const out = new Uint8ClampedArray(new ArrayBuffer(src.length));
  for (let i = 0; i < src.length; i++) {
    if (i % 4 === 3) { out[i] = src[i]; continue; }
...
export async function enhanceImage(imageUrl) {
  const img = await new Promise((resolve, reject) => {
    ...
    el.onerror = () => { const el2 = new Image(); el2.onload = () => resolve(el2); el2.onerror = reject; el2.src = imageUrl; };
    el.src = imageUrl;
  });
...
export function isEnhanceableImage(fileType) {
